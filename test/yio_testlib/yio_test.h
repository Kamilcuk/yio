...
#else
static int re_match(const char *pat, const char *str, int *len) {
	(void)pat; (void)str; (void)len;
	return -1;
}
#endif
...
#define YIO_TEST(shouldbe, fmt, ...)  do { \
		char buf[1024]; \
		const int err = yio_snprint(buf, sizeof(buf), fmt, ## __VA_ARGS__); \
