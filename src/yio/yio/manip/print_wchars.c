...
static int wstr_to_yyiostring(const wchar_t *ws, size_t ws_maxlen, YIO_buf *out) {
	mbstate_t state;
	...
	while (psrc != NULL && ws_maxlen > 0) {
		...
		const size_t cap = YIO_buf_free_size(out);
		const wchar_t *const psrc_old = psrc;
		const size_t r = wcsnrtombs(YIO_buf_data(out) + YIO_buf_len(out), &psrc, ws_maxlen, cap, &state);
	...
	for (size_t i = 0; i < ws_maxlen && ws[i] != L'\0'; ++i) {
		...
		const size_t r = wcrtomb(YIO_buf_data(out) + YIO_buf_len(out), ws[i], &state);
		if (r == (size_t)-1) return YIO_ERROR_WCRTOMB;
		YIO_buf_set_used(out, YIO_buf_len(out) + r);
	}
#endif
...
int YIO_print_constwcharpnt(yio_printctx_t *t) {
	...
	if (ret == 0) {
		ret = yio_printctx_put(t, YIO_buf_data(&out), YIO_buf_len(&out));
	}
	YIO_buf_fini(&out);
	return ret;
}
