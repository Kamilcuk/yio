...
static int c16str_to_yyiostring(const char16_t *str, size_t maxlen, YIO_string *out) {
	mbstate_t state;
	...
	for (size_t i = 0; i < maxlen && str[i] != u'\0'; ++i) {
		...
		const size_t r = c16rtomb(YIO_string_data(out) + YIO_string_len(out), str[i], &state);
...
static int ustr_to_yyiostring(const char32_t *str, size_t maxlen, YIO_string *out) {
	mbstate_t state;
	memset(&state, 0, sizeof(state));
	const size_t mb_cur_max = MB_CUR_MAX;
	for (size_t i = 0; i < maxlen && str[i] != U'\0'; ++i) {
		...
		const size_t r = c32rtomb(YIO_string_data(out) + YIO_string_len(out), str[i], &state);
		if (r == (size_t)-1) return YIO_ERROR_WCRTOMB;
		if (r != (size_t)-2 && r != (size_t)-3) {
			YIO_string_set_used(out, YIO_string_len(out) + r);
		}
...
int YIO_print_constchar16pnt(yio_printctx_t *t) {
	...
	if (err == 0) {
		err = yio_printctx_put(t, YIO_string_data(&out), YIO_string_len(&out));
	}
	YIO_string_fini(&out);
	return err;
}
