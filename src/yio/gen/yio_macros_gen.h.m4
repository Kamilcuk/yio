...
m4_forloopX(6, m4_N, `ifelse(eval(X % 2), 0, `
#define _yIO_GENERIC_GEN_`'X`'(m4_forloopY(1, eval(X / 2), `_`'Y`'a,_`'Y`'b', `,'))  `\'
		m4_forloopY(1, eval(X / 2), `_`'Y`'a:_`'Y`'b', `,')')')
dnl';
#define _yIO_GENERIC_GEN_N(m4_seqdashcomma(1,m4_N),N,...)  \
		_yIO_GENERIC_GEN_##N
#define _yIO_GENERIC_GEN(...)  \
		_yIO_GENERIC_GEN_N(__VA_ARGS__,m4_seqcomma(m4_N,0))(__VA_ARGS__)
...
#define _yIO_APPLYFOREACH_N(m4_seqdashcomma(0,m4_N),N,...)  \
		_yIO_APPLYFOREACH_##N
#define _yIO_APPLYFOREACH(...)  \
		_yIO_APPLYFOREACH_N(__VA_ARGS__,m4_seqcomma(m4_N,0))(__VA_ARGS__)
...
#define _yIO_APPLYFOREACHCOMMA_N(m4_seqdashcomma(0, m4_N),N,...)  \
		_yIO_APPLYFOREACHCOMMA_##N
#define _yIO_APPLYFOREACHCOMMA(...)  \
		_yIO_APPLYFOREACHCOMMA_N(__VA_ARGS__,m4_seqcomma(m4_N, 0))(__VA_ARGS__)
...
#define _yIO_APPLYFOREACHSEP_N(m4_seqdashcomma(0, m4_N),N,...)  \
		_yIO_APPLYFOREACHSEP_##N
#define _yIO_APPLYFOREACHSEP(callback, ...)  \
		_yIO_APPLYFOREACHSEP_N(__VA_ARGS__,m4_seqcomma(m4_N, 0))(callback, __VA_ARGS__)
...
#define _yIO_JAPPLYFOREACH_N(m4_seqdashcomma(0, m4_N),N,...)  \
		_yIO_JAPPLYFOREACH_##N
#define _yIO_JAPPLYFOREACH(before, after, ...)  \
		_yIO_JAPPLYFOREACH_N(__VA_ARGS__,m4_seqcomma(m4_N, 0))(before, after, __VA_ARGS__)
...
#define _yIO_APPLYFOREACHUNPACK_N(m4_seqdashcomma(0,m4_N),N,...)  \
		_yIO_APPLYFOREACHUNPACK_##N
#define _yIO_APPLYFOREACHUNPACK(...)  \
		_yIO_APPLYFOREACHUNPACK_N(__VA_ARGS__,m4_seqcomma(m4_N,0))(__VA_ARGS__)
...
#define _yIO_APPLYFOREACHUNPACKCOMMA_N(m4_seqdashcomma(0,m4_N),N,...)  \
		_yIO_APPLYFOREACHUNPACKCOMMA_##N
#define _yIO_APPLYFOREACHUNPACKCOMMA(...)  \
		_yIO_APPLYFOREACHUNPACKCOMMA_N(__VA_ARGS__,m4_seqcomma(m4_N,0))(__VA_ARGS__)
...
#define _yIO_STRLEN(s) (\
		!(s)[0]?0:!(s)[1]?1:\
		m4_forloopX(2, 120,``!(s)['X`]?'X`:'ifelse(eval(X%5),`1',`\
		')') \
		-1)
		
define(`m4_SLOTS_END', `199')dnl;
...
#define _yIO_PRINT_SLOTS()  m4_forloopX(100, m4_SLOTS_END, ` YIO_PRINT_SLOT_`'X`'()')dnl';

#define _yIO_SCAN_SLOTS()   m4_forloopX(100, m4_SLOTS_END, ` YIO_SCAN_SLOT_`'X`'()')dnl';
