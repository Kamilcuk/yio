...
class MyEnvironment(jinja2.Environment):
    def __init__(self, srcdir, cachedir, defines, *args, **kwargs):
        ...
        self.preprocess = lambda source, name, filename: preprocess_source(
            source, filename
        )
        self.dependencies = []
...
def run_task(srcdir, cachedir, defines, mode, source, output_file):
    ...
    header = (
        "// Do not edit! Auto-generated by preprocess.py\n"
        f"// mode = {mode}\n"
        f'#line 1 "{template.filename}"\n'
    )
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default=[],
        action="append",
        help="Source directory for template lookup.",
    )
    parser.add_argument(
        "-D",
        "--define",
        default=[],
        action="append",
        help="Define variables for Jinja2 environment.",
    )
    parser.add_argument("-C", "--cachedir", help="Directory for bytecode cache.")
    parser.add_argument("-d", "--depfile", help="Path to generate a dependency file.")
    parser.add_argument("-m", "--mode", default="yio", help="Preprocessing mode.")
    parser.add_argument(
        "-j", "--jobs", type=int, default=1, help="Number of concurrent jobs."
    )
    parser.add_argument(
        "--debug", action="store_true", help="Enable debug mode (legacy)."
    )
    parser.add_argument(
        "--task",
        action="append",
        ...
        help="Specify a source and output file task.",
    )
    parser.add_argument("source", nargs="?", help="Source template file.")
    parser.add_argument("output", nargs="?", help="Output C/H file.")
...
if __name__ == "__main__":
    ...
    tasks = (
        args.task
        if args.task
        else ([[args.source, args.output]] if args.source else [])
    )
    ...
    if args.jobs > 1 and len(tasks) > 1:
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.jobs) as ex:
            futures = [
                ex.submit(
                    run_task, args.srcdir, args.cachedir, defines, args.mode, t[0], t[1]
                )
                for t in tasks
            ]
        ...
        for t in tasks:
            out_file, deps = run_task(
                args.srcdir, args.cachedir, defines, args.mode, t[0], t[1]
            )
            all_dependencies[out_file] = deps
    ...
    if args.depfile:
        ...
        with open(args.depfile, "w") as f:
            for out_file, deps in sorted(all_dependencies.items()):
                deps_str = " ".join(
                    sorted([os.path.realpath(d).replace(" ", "\\ ") for d in deps])
                )
                f.write(
                    f"{os.path.realpath(out_file).replace(' ', '\\ ')}: {deps_str}\n"
                )
