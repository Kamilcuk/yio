...
- `YYIO_FLOAT<suffix>`: The C type (e.g., `float`, `double`).
- `YYIO_modf<suffix>`: Suffix-based `modf` function.
- `YYIO_frexp<suffix>`: Suffix-based `frexp` function.
- `YYIO_floor<suffix>`: Suffix-based `floor` function.
- `YYIO_fabs<suffix>`: Suffix-based `fabs` function.
- `YYIO_pow<suffix>`: Suffix-based `pow` function.
- `YYIO_exp10<suffix>`: Suffix-based `exp10` function (emulated if not available).
- `YYIO_FLOAT_MANT_DIG<suffix>`: Number of base-FLT_RADIX digits in the mantissa.
- `YYIO_FLOAT_MAX<suffix>`: Maximum value of the type.
- `YYIO_FLOAT_MIN<suffix>`: Minimum normalized positive value.
- `YYIO_FLOAT_EPSILON<suffix>`: Difference between 1.0 and the next representable value.
- `YYIO_FLOAT_HUGE_VAL<suffix>`: Large positive value, used for overflow.
...
- `YYIO_RP_OF_<suffix>_IS_<RP>`: Defined to 1 if the type has the given representation.
- `YYIO_FLOAT_RP_<RP>`: The canonical type for the representation.
- `YYIO_modf_RP_<RP>`: Math functions for the representation.
- `YYIO_frexp10_RP_<RP>`: Base-10 frexp implementation for the representation.
...
Availability macros: `YYIO_has_float_astrfrom_<backend>_<repr>`.
Function signature: `int YYIO_float_astrfrom_<backend>_<repr>(YYIO_string *v, int precision, char spec, YYIO_FLOAT_RP_<repr> val)`.
